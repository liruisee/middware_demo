...
LOGGING = {
    ...
    "formatters": {
        ...
        'simple_spcial': {
            'format': '%(asctime)s %(levelname)s [%(filename)s:'
                      '%(lineno)d] %(name)s - \x01\x01%(message)s'
        },
        'standard': {
            # 较为复杂的输出模式，可以进行自定义
            ...
            'format': '%(asctime)s %(levelname)s processId:%(process)d:|file:%(filename)s|lineno:'
...
class Logger:
    ...
    def __init__(self, app_name: str, log_path: str, is_debug: bool, is_write_file: bool, level: str, is_spcial: bool):
        ...
        assert(level.upper() in ('DEBUG', 'INFO', 'WARN', 'ERROR', 'CRITICAL')), 'level 必须为DEBUG, INFO, WARN, ERROR, CRITICAL其中之一'
        # 防止再次实例化
        self.__is_init = True
        ...
        if is_debug:
            ...
            if is_spcial:
                LOGGING['handlers'][handler_name] = self.get_spcial_console_handler_conf()
            else:
    ...
    # 控制spcial台输出handler配置
    def get_spcial_console_handler_conf(self):
        console_handler_conf = {
            # 定义输出流的类
            "class": "logging.StreamHandler",
            # handler等级，如果实际执行等级高于此等级，则不触发handler
    ...
    # 写入文件handler配置
    def get_file_handler_conf(self, filename: str, level: str):
        file_handler_conf = {
            # 定义写入文件的日志类，此类为按时间分割日志类，还有一些按日志大小分割日志的类等
            "class": "mlogging_handlers.TimedRotatingFileHandlerMP",
            # 日志等级
        ...
        update_dict = {'filename': filename, 'level': self.level, 'filters': filters}
...
# 获取日常logger
def get_logger(app_name: str, log_path: str=None, is_debug=True, is_write_file=False, level='INFO', is_spcial=False):
    Logger(log_path=log_path, app_name=app_name, is_debug=is_debug, is_write_file=is_write_file, level=level, is_spcial=is_spcial)
...
# 获取request logger
def get_request_logger(app_name: str, log_path: str=None, is_debug=True, is_write_file=False, level='INFO', is_spcial=False):
    Logger(app_name=app_name, log_path=log_path, is_debug=is_debug, is_write_file=is_write_file, level=level, is_spcial=is_spcial)
...
if __name__ == '__main__':
    # 临时目录
    log_root_path = '../log_tool'
    # 单例模式测试
    logger = get_logger(app_name='devenv', log_path=log_root_path, is_debug=True, is_write_file=True)
